...
def generate_test_suite_java(
    spec: ProgramSpec,
    program_data: dict = None,
    package: str = "com.modernization.generated",
    main_class: str = "Main",
) -> str:
    """Generate a JUnit 5 test class from a program spec (W7).

    Mirrors `generate_test_suite` (which produces pytest) but emits Java tests.
    Uses the same internal `GeneratedTestCase` extraction so the test
    *structure* is identical across languages — same scenario count, same
    categories, same coverage of paragraphs/CICS ops/decision branches.

    Each test gets:
      - @Test annotation
      - A descriptive @DisplayName built from the case description
      - assertNotNull(program) as the baseline always-pass assertion
      - Original Python assertions preserved as // comments so the human
        implementing the body knows what was intended
    """
    class_name = _cobol_name_to_class(spec.program)
    ...
    lines.append("")
    lines.append("import static org.junit.jupiter.api.Assertions.assertDoesNotThrow;")
    lines.append("import static org.junit.jupiter.api.Assertions.assertNotNull;")
    ...
    lines.append(f" * Paragraphs: {spec.paragraph_count}")
    lines.append(
        f" * Complexity: {spec.cyclomatic_complexity} ({spec.complexity_grade})"
    )
    lines.append(" *")
    lines.append(" * Generated by Masquerade COBOL Intelligence Engine.")
    lines.append(" * Test names mirror the pytest version 1:1 so cross-language")
    lines.append(" * coverage can be compared scenario-by-scenario.")
    lines.append(" */")
    lines.append(f"@DisplayName(\"{class_name} ({spec.program_type}, {spec.code_lines} LOC)\")")
    lines.append(f"class {test_class} {{")
    lines.append("")
    lines.append(f"    private {main_class} program;")
    lines.append("")
    lines.append("    @BeforeEach")
    lines.append("    void setUp() {")
    lines.append(f"        program = new {main_class}();")
    lines.append("    }")
    lines.append("")

    for tc in all_tests:
        java_method = _to_java_test_method(tc.name)
        lines.append(f"    @Test")
        # JUnit 5 uses @DisplayName for human-readable scenario titles
        safe_desc = tc.description.replace('"', '\\"').replace("\n", " ")
        lines.append(f'    @DisplayName("{safe_desc[:120]}")')
        if tc.name in behavioral_names:
            lines.append('    @Disabled("skeleton not yet implemented")')
        lines.append(f"    void {java_method}() {{")
        # Mocks: emit as commented hints (Mockito setup is left to human)
        for mock_line in tc.mocks:
            lines.append(f"        // mock: {mock_line}")
        # Setup: emit as commented preconditions
        for setup_line in tc.setup:
            lines.append(f"        // setup: {setup_line}")
        # Assertions: translate Python patterns to real JUnit where possible.
        # Patterns that produce real assertions:
        #   "assert program is not None" → assertNotNull(program)
        #   "assert hasattr(program, 'X')" → reflection-based field/method check
        #   "assert callable(getattr(program, 'X'))" → same, already covered
        # Everything else → comment + assertNotNull fallback
        has_real_assertion = False
        for assertion in tc.assertions:
            java_assertion = _translate_assertion_to_java(assertion)
            if java_assertion:
                lines.append(f"        {java_assertion}")
                has_real_assertion = True
            else:
                lines.append(f"        // intent: {assertion}")
        if not has_real_assertion:
            lines.append("        assertNotNull(program);")
        lines.append("    }")
        lines.append("")
...
def _translate_assertion_to_java(assertion: str) -> Optional[str]:
    """Translate a Python assertion string to a JUnit 5 assertion.

    Returns the Java assertion line, or None if the assertion can't be
    automatically translated (will be emitted as a comment instead).
    """
    import re
    ...
    if a == "assert program is not None":
        return "assertNotNull(program);"

    # "assert hasattr(program, 'method_name')"
    m = re.match(r"assert hasattr\(program, '(\w+)'\)", a)
    if m:
        attr = m.group(1)
        java_name = _to_java_test_method(attr)  # snake→camel
        # Try as method first (paragraphs), field as fallback (copybooks)
        return (
            f'assertDoesNotThrow(() -> '
            f'{{ try {{ program.getClass().getMethod("{java_name}"); }} '
            f'catch (NoSuchMethodException e) '
            f'{{ program.getClass().getDeclaredField("{java_name}"); }} }}, '
            f'"{java_name} should exist");'
        )

    # "assert callable(getattr(program, 'X'))" — redundant if hasattr passed
    m = re.match(r"assert callable\(getattr\(program, '(\w+)'\)\)", a)
    if m:
        attr = m.group(1)
        java_name = _to_java_test_method(attr)
        return f"assertTrue(true, \"{java_name} is callable — compile-time verified\");"

    # "assert True  # ..." — placeholder, emit as assertTrue with the comment
    if a.startswith("assert True"):
        comment = a[len("assert True"):].strip().lstrip("#").strip()
        safe = comment.replace('"', '\\"')[:100]
        return f'assertTrue(true, "{safe}");'

    # Can't translate — return None so caller emits as comment
    return None


def _to_java_test_method(test_name: str) -> str:
    """Convert a snake_case pytest name (test_foo_bar) to camelCase JUnit
    (testFooBar)."""
